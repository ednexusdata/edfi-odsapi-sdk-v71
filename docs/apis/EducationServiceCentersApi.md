# EdFi.OdsApi.Sdk.v71.Apis.All.EducationServiceCentersApi
...
> List&lt;EdFiEducationServiceCenter&gt; GetEducationServiceCenters (int offset = null, int limit = null, long minChangeVersion = null, long maxChangeVersion = null, bool totalCount = null, long educationServiceCenterId = null, long stateEducationAgencyId = null, bool useSnapshot = null)
...
namespace Example
{
    public class GetEducationServiceCentersExample
    {
        public static void Main()
        {
            ...
            var maxChangeVersion = 789L;  // long | Used in synchronization to set sequence maximum ChangeVersion (optional) 
            var totalCount = false;  // bool | Indicates if the total number of items available should be returned in the 'Total-Count' header of the response.  If set to false, 'Total-Count' header will not be provided. (optional)  (default to false)
            var educationServiceCenterId = 789L;  // long | The identifier assigned to an education service center. (optional) 
            var stateEducationAgencyId = 789L;  // long | The identifier assigned to a state education agency. (optional) 
            var useSnapshot = false;  // bool | Indicates if the configured Snapshot should be used. (optional)  (default to false)

            try
            {
                // Retrieves specific resources using the resource's property values (using the \"Get\" pattern).
                List<EdFiEducationServiceCenter> result = apiInstance.GetEducationServiceCenters(offset, limit, minChangeVersion, maxChangeVersion, totalCount, educationServiceCenterId, stateEducationAgencyId, useSnapshot);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
...
{
    // Retrieves specific resources using the resource's property values (using the \"Get\" pattern).
    ApiResponse<List<EdFiEducationServiceCenter>> response = apiInstance.GetEducationServiceCentersWithHttpInfo(offset, limit, minChangeVersion, maxChangeVersion, totalCount, educationServiceCenterId, stateEducationAgencyId, useSnapshot);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}
